...
process BCFTOOLS_NORM {
    ...
    input:
    tuple val(sample), path(dipcall_vcf)
    path ref_fasta
    path ref_fai

    output:
    tuple val(sample), path("${sample}.${ref_fasta.baseName}.dipcall.normalized.vcf.gz"), path("${sample}.${ref_fasta.baseName}.dipcall.normalized.vcf.gz.tbi")

    script:
    """
    bcftools norm -m -any -f ${ref_fasta} -c s -Ou ${dipcall_vcf} \
        | bcftools view -f '.' -i 'GT~"1" & STRLEN(REF)<=1000 & STRLEN(ALT)<=1000' -Ou \
        | bcftools sort --write-index=tbi -Oz -o ${sample}.${ref_fasta.baseName}.dipcall.normalized.vcf.gz
    """
}
...
workflow {
    ...
    Channel
        ...
        .map { row -> tuple(row.sample, file(row.region_bed), file(row.region_tbi), file(row.dipcall_vcf)) }
        .set { samples_ch }

    BCFTOOLS_NORM(samples_ch.map { sample, region_bed, region_tbi, dipcall_vcf -> tuple(sample, dipcall_vcf) }, file(params.ref_fasta), file(params.ref_fai))
    ...
    annotate_input = samples_ch
        ...
        .map { sample, region_bed, region_tbi, dipcall_vcf, decomposed_vcf -> tuple(sample, region_bed, region_tbi, decomposed_vcf) } 
