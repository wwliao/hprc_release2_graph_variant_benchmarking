...
process BCFTOOLS_NORM {
    ...
    output:
    tuple val(sample), path("${sample}.${ref_fasta.baseName}.mc_vcfwave.normalized.vcf.gz"), path("${sample}.${ref_fasta.baseName}.mc_vcfwave.normalized.vcf.gz.tbi")
    ...
    bcftools view -a -I -s ${sample} -Ou ${vcf} \
        ...
        | bcftools sort --write-index=tbi -Oz -o ${sample}.${ref_fasta.baseName}.mc_vcfwave.normalized.vcf.gz
    """
}
